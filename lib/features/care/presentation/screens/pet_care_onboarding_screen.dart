...
class _PetCareOnboardingScreenState
  ...
  void _fillCustomFields(Map<String, dynamic> data) {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      [_custom0S, _custom1S, _custom2S][i].text = sub != null && sub.isNotEmpty
          ? sub
          : defaultsS[i];
    }
  }
  ...
  Future<void> _save({required bool done}) async {
    if (done && _useCustomChecklist) {
      ...
      if (c == null || c.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Add at least one custom task or turn off the switch.',
            ),
          ),
        );
        return;
    ...
    try {
      await petCareRepository.saveOnboarding(
        widget.petId,
        _toJson(),
        markComplete: done,
      );
  ...
  List<Widget> _buildStep1() {
    return [
      const _StepDescription(
        'Tell us about your pet so we can personalize their care plan.',
      ),
  ...
  List<Widget> _buildStep2() {
    return [
      const _StepDescription(
        'Understanding your pet\'s personality helps us suggest the right activities and routines.',
      ),
      const SizedBox(height: 16),
  ...
  List<Widget> _buildStep3() {
    ...
    return [
      const _StepDescription(
        'Health details help us tailor feeding, medication reminders, and activity suggestions.',
      ),
      const SizedBox(height: 16),
  ...
  List<Widget> _buildStep4() {
    ...
    return [
      const _StepDescription(
        'Set your primary care goal and customize your daily checklist.',
      ),
      const SizedBox(height: 16),
