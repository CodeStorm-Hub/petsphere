...
class MatchRepository {
  static bool _isRejectionStillInCooldown(Map<String, dynamic> row) {
    ...
    final rejectedAt = raw != null ? DateTime.tryParse(raw)?.toUtc() : null;
    ...
    return DateTime.now().toUtc().isBefore(
      anchor.add(kDiscoveryRejectionCooldown),
    );
  }
  ...
  Future<List<PetModel>> fetchDiscoveryPets({
    ...
    final requestedRows = await supabase
        .from('match_requests')
        .select(
          'sender_pet_id, receiver_pet_id, status, rejected_at, created_at',
        )
  ...
  // -------------------------------------------------------------------------
  Future<List<MatchRequestModel>> fetchAllMyRequests(
