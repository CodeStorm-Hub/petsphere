...
int calculateDosesPerDay(String frequency) {
  ...
  if (lower.contains('twice') || lower.contains('2x') || lower.contains('bid')) {
    return 2;
  }
  if (lower.contains('thrice') || lower.contains('3x') || lower.contains('tid')) {
    return 3;
  }
  if (lower.contains('four') || lower.contains('4x') || lower.contains('qid')) {
    return 4;
  }
  // Default: once daily
  return 1;
...
bool areMedicationDosesLow(
  ...
  final futureDoses = doses.where(
    (d) => d.givenAt == null && d.scheduledFor.isAfter(now),
  );
...
List<MedicationDose> getOverdueDoses(List<MedicationDose> doses) {
  final now = DateTime.now();
...
HealthMetricsSummary calculateHealthMetrics({
  ...
  final nextVaccination =
      getUpcomingVaccinations(
        vaccinations,
        lookAhead: const Duration(days: 365),
      ).fold<DateTime?>(null, (earliest, current) {
        final due = current.nextDueDate;
        if (due == null) return earliest;
        if (earliest == null) return due;
        return due.isBefore(earliest) ? due : earliest;
      });
