  ...
  Future<void> _publish(List<PetModel> pets) async {
    ...
    try {
      ...
      final success = await ref
          ...
          .addStory(pet, mediaUrl, _captionController.text.trim());
      if (!mounted) return;

      if (success) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Story posted as ${pet.name}.')));
        context.pop();
      } else {
          ...
          : DecoratedBox(
              ...
              child: LayoutBuilder(
                builder: (context, _) {
                  ...
                  final mediaCardHeight = (screenHeight * 0.46).clamp(
                    260.0,
                    460.0,
                  );
