...
class BootstrapState {
  const BootstrapState({this.lastHydratedUserId, this.isRefreshing = false});
...
///
/// [petProvider], [notificationProvider], [careLogProvider], [careGoalsProvider],
/// [careGamificationProvider], [vitalsProvider],
/// [medicationProvider], [appointmentProvider], [vaccinationProvider],
/// [parasiteProvider], [dentalProvider], [allergyProvider].
///
  ...
  Future<void> _performDataRefresh(String userId, {required bool force}) async {
    ...
    try {
      if (!force && state.lastHydratedUserId == userId) {
        AppLogger.debug('${AppStrings.bootstrapSkipHydrate} for $userId', tag: 'BootstrapNotifier');
        return;
      }
      ...
      AppLogger.debug(
        '${AppStrings.bootstrapHydratingData}: $userId (force=$force, accountSwitch=$isAccountSwitch)',
        tag: 'BootstrapNotifier',
      );
...
final bootstrapProvider = NotifierProvider<BootstrapNotifier, BootstrapState>(
