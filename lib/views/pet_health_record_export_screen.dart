...
class PetHealthRecordExportScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PetHealthRecordExportScreen> createState() => _PetHealthRecordExportScreenState();
}

class _PetHealthRecordExportScreenState extends ConsumerState<PetHealthRecordExportScreen> {
  ...
  void _handleExport(String action) async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Health record successfully ${action == "share" ? "shared" : "downloaded"}!'),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text('Select Categories', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900, letterSpacing: -0.5)),
                    const Spacer(),
                    TextButton.icon(
                      onPressed: () {
                        setState(() {
                      ...
                      icon: Icon(_options.values.every((v) => v) ? Icons.deselect_rounded : Icons.select_all_rounded, size: 18),
                      label: Text(_options.values.every((v) => v) ? 'Deselect All' : 'Select All', style: const TextStyle(fontWeight: FontWeight.w700)),
                    ),
                  ],
                ),
                    ...
                    children: _options.keys.map((key) {
                      ...
                      return Column(
                        children: [
                          CheckboxListTile(
                            value: _options[key],
                            onChanged: (v) => setState(() => _options[key] = v ?? false),
                            title: Text(key, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                            activeColor: colorScheme.primary,
                            checkboxShape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
                            contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 4),
                            controlAffinity: ListTileControlAffinity.trailing,
                          ),
                          if (!isLast) Divider(indent: 20, endIndent: 20, height: 1, color: colorScheme.outlineVariant.withAlpha(100)),
                        ],
                      );
                    }).toList(),
                  ),
        ...
        children: [
          if (isGenerating)
            const SizedBox(
              height: 72,
              width: 72,
              child: CircularProgressIndicator(strokeWidth: 8, strokeCap: StrokeCap.round),
            )
          else
            Container(
              padding: const EdgeInsets.all(20),
              decoration: BoxDecoration(color: colorScheme.primary.withAlpha(30), shape: BoxShape.circle),
              child: Icon(Icons.picture_as_pdf_rounded, size: 56, color: colorScheme.primary),
            ),
          ...
          Container(
            ...
            child: Text(
              isGenerating ? 'Generating document...' : 'Approx. 2.4 MB • 12 Pages',
          ...
          FilledButton.icon(
            ...
            label: const Text('Preview Content', style: TextStyle(fontWeight: FontWeight.w800)),
...
class _FormatSelectorState extends State<_FormatSelector> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text('File Format', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        SegmentedButton<String>(
          segments: const [
            ButtonSegment(value: 'PDF', label: Text('PDF'), icon: Icon(Icons.picture_as_pdf, size: 16)),
            ButtonSegment(value: 'CSV', label: Text('CSV'), icon: Icon(Icons.table_chart, size: 16)),
            ButtonSegment(value: 'JSON', label: Text('JSON'), icon: Icon(Icons.code, size: 16)),
          ],
          selected: {_format},
